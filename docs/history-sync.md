...
O evento `connection.update` com `close` atualiza imediatamente o estado em memória e encerra o socket. Logout pelo celular passa diretamente para `REMOVED`; eventos `open` ou QR atrasados daquele socket não podem reativá-lo. O webhook `connection.close`/`connection.removed` é persistido diretamente no recebimento, fora da fila de importação. As pendências de histórico e mídia daquela conexão são descartadas; uma escrita SQL já em andamento pode terminar, e dados já persistidos são preservados. A API confere o estado antes das próximas etapas de importação, resincronização e entrega de dados.
