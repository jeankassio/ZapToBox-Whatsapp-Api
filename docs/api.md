...
Resposta: `{success:true,message,messageId,key,data:WAMessage,syncPending?:true}`. O ID é gerado pelo Baileys antes do envio; sucesso indica que a operação de encaminhamento terminou, sem confirmar entrega ao aparelho destinatário. Confirmações e falhas posteriores chegam por `messages.update` e `message-receipt.update`. `syncPending:true` indica encaminhamento concluído com falha posterior na gravação/publicação local; **não repita automaticamente**. Não há chave HTTP de idempotência para envio. Um timeout também pode acontecer depois de o WhatsApp aceitar a mensagem.

Gravações do navegador devem ser convertidas no backend antes de chamar `sendAudio`: envie bytes reais Ogg/Opus mono com `mimetype:"audio/ogg; codecs=opus"` e `ptt:true` para mensagem de voz. A API verifica o cabeçalho Ogg/Opus e o canal mono para PTT, mas não decodifica nem converte áudio. WebM/Matroska, WAV, FLAC, arquivos vazios e PTT incompatível retornam 415 antes do envio, inclusive quando esses contêineres são rotulados como Ogg. Áudios comuns MP3/MP4/AAC/AMR continuam aceitos sem PTT. Alterar somente a extensão ou o MIME não converte os bytes. A recomendação de Ogg/Opus mono segue o [provedor Baileys rc14](https://github.com/WhiskeySockets/Baileys/blob/v7.0.0-rc14/README.md#audio-message).
