...
| `messaging-history.progress` | Campos restantes do histórico: isLatest?, progress?, syncType?, chunkOrder?, peerDataRequestSessionId?, pastParticipants?, lidPnMappings?. |
...
Arrays grandes em data são divididos em chunks: até 500 itens, separando um novo lote quando a soma dos itens ultrapassaria 8.000.000 bytes. Um item individual maior que esse alvo permanece inteiro e pode exigir limite maior no receptor; não é truncado. Cada chunk recebe seu ID e pode ser repetido independentemente. Um POST não representa necessariamente o histórico inteiro.
