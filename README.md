...
Crie uma conexão pelo frontend ou por `POST /instances/create`, conforme a [documentação HTTP completa](docs/api.md). A nova sessão precisa ser pareada por QR no aplicativo WhatsApp. A API também aceita `phoneNumber` para solicitar código. Sucesso HTTP na criação não significa conta online: aguarde `connection.open`.
