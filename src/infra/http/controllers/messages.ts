import { delay, type AnyMessageContent, type MiscMessageGenerationOptions, type WAMessageKey } from '@whiskeysockets/baileys';
...
type MessageDependencies = ControllerDependencies & { fetchMedia?: (url: string) => Promise<Buffer> };
export default class MessagesController extends SocketController {
  ...
  async sendMessageLocation(message: LocationMessage, options?: Options) { return this.sendMessage({ location: { degreesLatitude: message.location.degreesLatitude, degreesLongitude: message.location.degreesLongitude } }, await this.filterOptions(options)); }
  ...
  async sendMessageSticker(message: StickerMessage, options?: Options) { return this.sendMessage({ sticker: await this.mediaBuffer(message.sticker.url) }, await this.filterOptions(options)); }
  ...
  async sendMessage(message: AnyMessageContent, options?: MiscMessageGenerationOptions): Promise<ControllerResult> {
    const result = await this.perform('Message sent successfully.', async sock => {
      ...
      const sent = await sock.sendMessage(this.jid, message, options);
