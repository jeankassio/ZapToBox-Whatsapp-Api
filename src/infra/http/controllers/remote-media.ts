...
function isTrustedOrigin(url: URL): boolean {
  return (process.env.TRUSTED_MEDIA_ORIGINS || '').split(',').map(value => value.trim()).filter(Boolean).some(value => {
    try {
      const allowed = new URL(value);
      return ['http:', 'https:'].includes(allowed.protocol) && !allowed.username && !allowed.password && !allowed.hash
        && allowed.pathname === '/' && !allowed.search && allowed.origin === url.origin;
    } catch { return false; }
  });
}
...
export async function downloadPublicMedia(rawUrl: string, redirects = 0): Promise<Buffer> {
  ...
  const trusted = isTrustedOrigin(url);
  const hostname = url.hostname.replace(/^\[|\]$/g, '');
  const addresses = isIP(hostname) ? [{ address: hostname, family: isIP(hostname) }] : await lookup(hostname, { all: true, verbatim: true });
  if (!addresses.length || (!trusted && addresses.some(entry => !isPublicAddress(entry.address)))) throw new RequestError(400, 'Media URL must resolve to a public address or an explicitly trusted origin.');
  ...
  return new Promise<Buffer>((resolve, reject) => {
    const request = (url.protocol === 'https:' ? httpsRequest : httpRequest)(url, {
      ...
      if ([301, 302, 303, 307, 308].includes(response.statusCode ?? 0) && response.headers.location) {
        // Do not consume an arbitrary redirect body or let URL parsing escape this callback.
        response.destroy();
        let destination: string;
        try { destination = new URL(response.headers.location, url).toString(); }
        catch { reject(new RequestError(400, 'Invalid media redirect URL.')); return; }
        void downloadPublicMedia(destination, redirects + 1).then(resolve, reject);
        return;
      }
