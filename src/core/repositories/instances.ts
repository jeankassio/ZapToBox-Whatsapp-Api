...
export default class InstancesRepository {
  async list(ownerFilter?:string): Promise<InstanceInfo[]> {
    ...
    for(const item of Object.values(instanceConnection)) known.set(instanceKey(item.owner,item.instanceName),{owner:item.owner,instanceName:item.instanceName});
    return [...known].filter(([,pair])=>!ownerFilter || pair.owner===ownerFilter).map(([key,pair])=>{
      const loaded=instanceConnection[key];
      return publicInstanceInfo(loaded ?? {...pair,connectionStatus:'OFFLINE'});
    });
  }
