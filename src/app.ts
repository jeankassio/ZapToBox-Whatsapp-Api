...
export function createApp(options: { token?:string; ready?:()=>Promise<void>; isReady?:()=>boolean } = {}) {
  const app = express();
  ...
  app.use((_req,res,next)=>{
    if(options.isReady&&!options.isReady()){res.status(503).json({success:false,error:'API is starting or shutting down'});return;}
    next();
  });
